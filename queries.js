...
const getFriends = (request, response) => {
  ...
  pool.query('SELECT * FROM users WHERE friendid = $1', [user], (error, results) => {
    if (error) {
      throw error
    }
...
const createUser = (request, response) => {
  const { user } = request.body
  pool.query('SELECT * FROM users WHERE tgid = $1', [user], (error, results) => {
    if (error) {
      throw error
    }
    if (results.rows.length === 0) {
      pool.query('INSERT INTO users (tgid, mount, friendid) VALUES ($1, $2, $3)', [user, 0, ""], (error, results) => {
        if (error) {
          throw error
        }
...
const bonus = (request, response) => {
  ...
  pool.query(`SELECT * FROM socials WHERE tgid = $1 AND title = $2 AND date >= CURRENT_TIMESTAMP - INTERVAL '24 hours'`, [user, title], (error, socialResults) => {
    if (error) throw error
    if (socialResults.rows.length > 0) return response.json({ stats: "error", message: "You need more time" });
    pool.query('SELECT * FROM users WHERE tgid = $1', [user], (error, results) => {
      if (error) throw error
      pool.query(
        'UPDATE users SET mount = $1 WHERE tgid = $2',
        [results.rows[0].mount + price, user],
        (error) => {
          if (error) {
            throw error
          }
          pool.query('INSERT INTO socials (tgid, title, date) VALUES ($1, $2, $3)', [user, title, currentDateTime], (error, results1) => {
            if (error) {
              throw error
            }
            return response.json({ stats: 'success', mount: results.rows[0].mount + price })
          })
        })
    })
  })
      ...
      if (!results.rows[0]?.publickey || !results.rows[0]?.privatekey) {
        ...
        pool.query(
          ...
          (error) => {
            ...
            return response.json({ publicKey: keypair.accountHex(), privateKey: _privateKey })
          })
      } else
        return response.json({ publicKey: results.rows[0].publickey, privateKey: results.rows[0].privatekey })
    }
  })
}
