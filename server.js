...
bot.command("start", async (ctx) => {
	...
	pool.query('SELECT * FROM users WHERE tgid = $1', [userid], async (error, results1) => {
		if (error) {
			throw error
		}
		...
		if (!user && receiveid) {
			pool.query('INSERT INTO users (tgid, mount, friendid) VALUES ($1, $2, $3)', [userid, ctx.from.is_premium === true ? 10000 : 5000, receiveid], async (error) => {
				if (error) throw error
				pool.query('SELECT * FROM users WHERE tgid = $1', [receiveid], (error, results2) => {
					if (error) throw error
					let sender = results2.rows[0];
					if (sender) sender.mount += ctx.from.is_premium === true ? 10000 : 5000;
					else {
						pool.query(
							'UPDATE users SET mount = $1 WHERE tgid = $2',
							[results2.rows[0].mount + ctx.from.is_premium === true ? 10000 : 5000, receiveid],
							(error) => {
								if (error) {
									throw error
								}
